...
/**
 * Orchestration script for running Privacy E2E tests with database reset.
 * 
...
try {
  ...
  // 4. Reset Assistant Service Database
  console.log('🔄 Seeding Assistant Service...');
  const assistantServiceDir = path.resolve(__dirname, '../../raices-assistant-service');
  ...
  console.log('📱 Launching Maestro Privacy E2E Suite...');
  const frontendDir = path.resolve(__dirname, '../');
  const privacyDir = path.join(frontendDir, 'e2e/04-privacy');
  const env = { ...process.env, EXPO_PUBLIC_USE_AUDIO_MOCK: 'true' };
  
  // Read directory, filter for yaml files, and sort alphabetically
  const files = fs.readdirSync(privacyDir)
    .filter(f => f.endsWith('.yaml') || f.endsWith('.yml'))
    .sort();

  for (const file of files) {
    console.log(`\n▶️ Running Flow: ${file}...`);
    execSync(`maestro test e2e/04-privacy/${file}`, { cwd: frontendDir, stdio: 'inherit', env });
  }
  
  console.log('\n🎉 All E2E Privacy tests completed successfully!');
