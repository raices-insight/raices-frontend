...
/**
 * Orchestration script for running Auth E2E tests with database reset.
 * 
...
try {
  ...
  console.log('📱 Launching Maestro Auth E2E Suite...');
  const frontendDir = path.resolve(__dirname, '../');
  const authDir = path.join(frontendDir, 'e2e/01-auth');
  ...
  for (const file of files) {
    console.log(`\n▶️ Running Flow: ${file}...`);
    execSync(`maestro test e2e/01-auth/${file}`, { cwd: frontendDir, stdio: 'inherit', env });
  }
  
  console.log('\n🎉 All E2E Auth tests completed successfully!');
